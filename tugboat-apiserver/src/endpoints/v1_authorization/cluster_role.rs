...
use tugboat_resources::manifests::authorization::v1::ClusterRole;
use utoipa::ToSchema;

#[utoipa::path(
        responses(
            (status = 200, description = "Resource created", body = ClusterRole),
            (status = 409, description = "Resource already exists", body = StatusResponse),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        request_body = ClusterRole
    )]
#[post("/apis/authorization/v1/clusterroles")]
pub(super) async fn handle_cluster_role_create(
    json: Json<ClusterRole>,
...
#[utoipa::path(
        responses(
            (status = 200, description = "Resource deleted", body = ClusterRole),
...
#[utoipa::path(
        responses(
            (status = 200, description = "List of resources", body = [ClusterRole]),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        params(
...
#[get("/apis/authorization/v1/clusterroles")]
pub(super) async fn handle_cluster_role_list(
    query: Query<ListQuery>,
    operator: Data<ApiOperator>,
) -> Result<HttpResponse, Box<StatusResponse>> {
    resource_handlers::list_resources::<ClusterRole>(&operator, query.into_inner(), None).await
}

#[utoipa::path(
        responses(
            (status = 200, description = "Resource details", body = ClusterRole),
            (status = 404, description = "Resource not found", body = StatusResponse),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        params(
...
#[utoipa::path(
        responses(
            (status = 200, description = "Resource updated", body = ClusterRole),
            (status = 404, description = "Resource not found", body = StatusResponse),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        params(
...
pub(super) async fn handle_cluster_role_replace(
    ...
    resource_handlers::replace_resource::<ClusterRole>(
        &operator,
        None,
...
pub(super) async fn handle_cluster_role_patch(
    ...
    resource_handlers::patch_resource::<ClusterRole>(
        &operator,
        None,
        path.into_inner().name,
        patch.into_inner(),
