...
use tugboat_resources::manifests::core::v1::StorageClass;
use utoipa::ToSchema;

#[utoipa::path(
        responses(
            (status = 200, description = "Resource created", body = StorageClass),
            (status = 409, description = "Resource already exists", body = StatusResponse),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        request_body = StorageClass
    )]
#[post("/api/v1/storageclasses")]
...
pub(super) struct StorageClassDeletePathParams {
    name: String,
}

#[utoipa::path(
        responses(
            (status = 200, description = "Resource deleted", body = StorageClass),
...
#[delete("/api/v1/storageclasses/{name}")]
...
#[utoipa::path(
        responses(
            (status = 200, description = "List of resources", body = [StorageClass]),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        params(
...
#[utoipa::path(
        responses(
            (status = 200, description = "Resource details", body = StorageClass),
            (status = 404, description = "Resource not found", body = StatusResponse),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        params(
