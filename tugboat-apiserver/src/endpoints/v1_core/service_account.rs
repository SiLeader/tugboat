...
#[utoipa::path(
        responses(
            (status = 200, description = "Resource created", body = ServiceAccount),
        ...
        request_body = ServiceAccount
    )]
#[post("/api/v1/namespaces/{namespace}/serviceaccounts")]
pub(super) async fn handle_service_account_create(
    path: Path<NamespacedPathParams>,
    json: Json<ServiceAccount>,
...
pub(super) async fn handle_service_account_delete(
    ...
    resource_handlers::delete_resource::<ServiceAccount>(
        &operator,
...
#[utoipa::path(
        responses(
            (status = 200, description = "List of resources", body = [ServiceAccount]),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        params(
            ("namespace" = String, Path, description = "Namespace of the resource"),
...
pub(super) async fn handle_service_account_list(
    path: Path<NamespacedPathParams>,
    query: Query<ListQuery>,
    operator: Data<ApiOperator>,
) -> Result<HttpResponse, Box<StatusResponse>> {
    resource_handlers::list_resources::<ServiceAccount>(
        &operator,
        query.into_inner(),
...
pub(super) async fn handle_service_account_list_all(
    query: Query<ListQuery>,
    operator: Data<ApiOperator>,
) -> Result<HttpResponse, Box<StatusResponse>> {
...
#[utoipa::path(
        responses(
            (status = 200, description = "Resource updated", body = ServiceAccount),
            (status = 404, description = "Resource not found", body = StatusResponse),
            (status = 500, description = "Internal server error", body = StatusResponse),
        ),
        params(
...
pub(super) async fn handle_service_account_replace(
    ...
    let path = path.into_inner();
    resource_handlers::replace_resource::<ServiceAccount>(
        &operator,
        Some(path.namespace),
        path.name,
...
pub(super) async fn handle_service_account_patch(
    ...
    let path = path.into_inner();
    resource_handlers::patch_resource::<ServiceAccount>(
        &operator,
        Some(path.namespace),
        path.name,
        patch.into_inner(),
