...
/// Detect and recover from partial publish failures.
///
/// When a CSI volume is mounted on the node but the state file write fails,
/// we detect this situation and allow recovery. On the next reconciliation,
/// this function validates the mounted volume and marks it as recovered
/// so the Ship can proceed (no permanent failure).
///
/// This is safe because:
/// - If volume is mounted and marked in CSI filesystem, it's real
/// - If volume is not mounted, node_volume_stats() will indicate it's not there
/// - Recovery allows retry of state persistence without blocking Ship
#[allow(dead_code)]
pub(crate) fn detect_and_recover_partial_published_volumes(
    ship_id: &str,
    mut persisted: Vec<PublishedVolume>,
    planned: &[PublishedVolume],
) -> Result<Vec<PublishedVolume>, ReconcileError> {
    ...
    // Exact match: recovery successful
    if persisted == planned_sorted {
        tracing::debug!(
            "Recovered CSI published volumes for ship '{}' match planned state",
            ship_id
        );
        return Ok(persisted);
    }

    // Mismatch: real corruption, cannot recover
    tracing::warn!(
        "Recovered CSI published volumes for ship '{}' do not match planned state ({}persisted vs {}planned)",
        ship_id,
        persisted.len(),
        planned_sorted.len()
    );
